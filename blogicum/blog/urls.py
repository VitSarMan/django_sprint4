...
urlpatterns = [
    ...
    path('user/<str:username>/', views.UserUpdateView.as_view(), name='edit_profile'),
    path('category/<str:category_slug>/', views.CategoryListView.as_view(), name='category_posts'),
    path('posts/<int:post_id>/comment/', views.CommentCreateView.as_view(), name='add_comment'),
    path('posts/detail/<int:post_id>/', views.PostDetailView.as_view(), name='post_detail'),
